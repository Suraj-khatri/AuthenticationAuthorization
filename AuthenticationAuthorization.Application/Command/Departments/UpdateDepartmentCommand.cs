...
public record UpdateDepartmentCommand(UpdateDepartmentsDTO dept) : IRequest<ApiResponse<GetDepartmentsDTO>>;

public class UpdateDepartmentCommandHandler : IRequestHandler<UpdateDepartmentCommand, ApiResponse<GetDepartmentsDTO>>
{
    ...
    public async Task<ApiResponse<GetDepartmentsDTO>> Handle(UpdateDepartmentCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var dept = _mapper.Map<Department>(request.dept);
            ...
            await _unitOfWork.DepartmentRepo.UpdateAsync(dept, cancellationToken);

...
public record AddBranchesCommand(AddBranchesDTO Addbranch) : IRequest<ApiResponse<GetBranchesDTO>>;

public class AddBranchesCommandHandler : IRequestHandler<AddBranchesCommand, ApiResponse<GetBranchesDTO>>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly IMapper _mapper;

    public AddBranchesCommandHandler(IUnitOfWork unitOfWork, IMapper mapper)
    {
        _unitOfWork = unitOfWork;
        _mapper = mapper;
    }

    public async Task<ApiResponse<GetBranchesDTO>> Handle(AddBranchesCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            branch.CreatedBy = 1000;
            branch.ModifiedBy = 1000;
            branch.CreatedDate = DateTime.UtcNow;
            branch.ModifiedDate = DateTime.UtcNow;

            await _unitOfWork.BranchRepo.InsertAsync(branch, cancellationToken);

            await _unitOfWork.SaveAsync(cancellationToken);

            var dto = _mapper.Map<GetBranchesDTO>(branch);
            return ApiResponse<GetBranchesDTO>.SuccessResponse(dto, "Added successfully", 201);
        }
